import { mat4, quat, vec2, vec3 } from 'https://cdn.jsdelivr.net/npm/gl-matrix@3.4.3/+esm';
...
function parseCssColorToRgba(cssColor, fallback = '#070b12') {
...
const BRAND_COLORS = ['#070b12', '#1F3556', '#C7A25D', '#38bdf8'];

function drawBrandedCell(ctx, x, y, cellSize, title, index) {
  const gradient = ctx.createLinearGradient(x, y, x + cellSize, y + cellSize);
  gradient.addColorStop(0, BRAND_COLORS[0]);
  gradient.addColorStop(0.55, BRAND_COLORS[1]);
  gradient.addColorStop(1, index % 2 === 0 ? BRAND_COLORS[2] : BRAND_COLORS[3]);
  ctx.fillStyle = gradient;
  ctx.fillRect(x, y, cellSize, cellSize);

  ctx.strokeStyle = BRAND_COLORS[2];
  ctx.lineWidth = 3;
  ctx.strokeRect(x + 28, y + 28, cellSize - 56, cellSize - 56);

  ctx.fillStyle = BRAND_COLORS[3];
  ctx.globalAlpha = 0.35;
  ctx.beginPath();
  ctx.arc(x + cellSize * 0.78, y + cellSize * 0.22, cellSize * 0.09, 0, Math.PI * 2);
  ctx.fill();
  ctx.globalAlpha = 1;
  ...
  const lineHeight = 48;
  const startY = y + cellSize / 2 - ((words.length - 1) * lineHeight) / 2;

  words.forEach((word, lineIndex) => {
    ctx.fillText(word, x + cellSize / 2, startY + lineIndex * lineHeight);
  });
}
...
class InfiniteGridMenu {
  ...
  #initTexture() {
    ...
    this.items.forEach((item, i) => {
      ...
      drawBrandedCell(ctx, x, y, cellSize, item.title, i);
    });
    ...
    const itemsWithImages = this.items
      .map((item, i) => ({ item, i }))
  ...
  #render() {
    ...
    const [r, g, b, a] = parseCssColorToRgba(bgVar || '#070b12');
...
function setOverlayState(elements, isMoving) {
  elements.forEach(el => {
    if (!el) return;
    el.classList.toggle('active', !isMoving);
    el.classList.toggle('inactive', isMoving);
  });
}

export function initInfiniteMenu(container, items, options = {}) {
  const canvas = container.querySelector('canvas.infinite-menu__canvas');
  if (!canvas) return null;

  const titleEl = container.querySelector('.infinite-menu__face-title');
  const descEl = container.querySelector('.infinite-menu__face-desc');
  const actionEl = container.querySelector('.infinite-menu__action');
  const overlayEls = [titleEl, descEl, actionEl];
  const scale = options.scale ?? 1;
  const menuItems = items.length ? items : defaultItems;

  let activeItem = menuItems[0] ?? null;
  let sketch = null;
  ...
  try {
    sketch = new InfiniteGridMenu(
      ...
      isMoving => setOverlayState(overlayEls, isMoving),
      sk => sk.run(),
      scale
    );
  } catch (error) {
  ...
  if (titleEl && activeItem) titleEl.textContent = activeItem.title || '';
  if (descEl && activeItem) descEl.textContent = activeItem.description || '';
  setOverlayState(overlayEls, false);
